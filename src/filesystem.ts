...
import { TarReader } from 'formats/tarReader';
...
export class NodePickedFile implements PickedFile {
	...
	async readTar(callback: (tar: TarReader) => Promise<void>): Promise<void> {
		// let fd: NodeFS.promises.FileHandle | null = null;
		try {
			return await callback(
				new TarReader(fs.createReadStream(this.filepath))
			);
		} finally {
			// await fd?.close();
		}
...
class FSReader extends Reader<NodeFS.promises.FileHandle> {
	...
	async readUint8Array(offset: number, length: number) {
		...
		return new Uint8Array(
			nodeBufferToArrayBuffer(buffer, 0, result.bytesRead)
		);
	}
}
