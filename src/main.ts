...
import YuqueImporter from 'formats/yuqueImporter';
...
export class ImportContext {
	...
	constructor(el: HTMLElement) {
		...
		this.progressBarEl = el.createDiv('importer-progress-bar', (el) => {
			this.progressBarInnerEl = el.createDiv(
		...
		el.createDiv('importer-stats-container', (el) => {
			el.createDiv('importer-stat mod-imported', (el) => {
				this.importedCountEl = el.createDiv({
					cls: 'importer-stat-count',
					text: '0',
				});
				el.createDiv({ cls: 'importer-stat-name', text: 'imported' });
			});
			el.createDiv('importer-stat mod-attachments', (el) => {
				this.attachmentCountEl = el.createDiv({
					cls: 'importer-stat-count',
					text: '0',
				});
				el.createDiv({
					cls: 'importer-stat-name',
					text: 'attachments',
				});
			});
			el.createDiv('importer-stat mod-remaining', (el) => {
				this.remainingCountEl = el.createDiv({
					cls: 'importer-stat-count',
					text: '0',
				});
			...
			el.createDiv('importer-stat mod-skipped', (el) => {
				this.skippedCountEl = el.createDiv({
					cls: 'importer-stat-count',
					text: '0',
				});
			...
			el.createDiv('importer-stat mod-failed', (el) => {
				this.failedCountEl = el.createDiv({
					cls: 'importer-stat-count',
					text: '0',
				});
	...
	reportSkipped(name: string, reason?: any) {
		...
		this.importLogEl.createDiv('list-item', (el) => {
			...
			el.createSpan({
				text:
					`"${truncateText(name, this.maxFileNameLength)}"` +
					(reason
						? ` because ${truncateText(
								String(reason),
	...
	reportProgress(current: number, total: number) {
		...
		this.progressBarInnerEl.style.width =
			((100 * current) / total).toFixed(1) + '%';
	}
...
export default class ImporterPlugin extends Plugin {
	...
	async onload() {
		this.importers = {
			...
			lakebook: {
				name: '语雀知识库',
				optionText: 'lakebook (.lakebook)',
				importer: YuqueImporter,
				helpPermalink: 'import/lakebook',
			},
			onenote: {
			...
			notion: {
				...
				formatDescription:
					'Export your Notion workspace to HTML format.',
			},
			'roam-json': {
				name: 'Roam Research',
				...
				formatDescription:
					'Export your Roam Research workspace to JSON format.',
			},
			textbundle: {
		...
		this.registerObsidianProtocolHandler('importer-auth', (data) => {
			...
			new Notice(
				'Unexpected auth event. Please restart the auth process.'
			);
		});
	...
	updateContent() {
		...
		if (selectedImporter.formatDescription) {
			...
			descriptionFragment.createSpan({
				text: selectedImporter.formatDescription,
			});
		}
		descriptionFragment.createEl('br');
		...
		if (selectedId && importers.hasOwnProperty(selectedId)) {
			let importer = (this.importer = new selectedImporter.importer(
				this.app,
				this
			));
				...
				el.createEl(
					...
					(el) => {
						el.addEventListener('click', async () => {
							...
							let ctx = (this.current = new ImportContext(
								progressEl
							));
							...
							let cancelButtonEl = buttonsEl.createEl(
								'button',
								{ cls: 'mod-danger', text: 'Stop' },
								(el) => {
